...
export default async function PostPage({
  ...
  return (
    <article className="space-y-8">
        ...
        {post.tags.length > 0 && (
          <div className="flex gap-2 flex-wrap pt-4">
            {post.tags.map((tag) => (
              <span
                key={tag}
                className="text-sm px-3 py-1 bg-[rgba(77,208,225,0.1)] border border-[rgba(77,208,225,0.3)] rounded text-[#4dd0e1]"
              >
                {tag}
              </span>
            ))}
      ...
      {/* Post Content */}
      <Card>
        <div className="text-[#e8e6e3] leading-relaxed space-y-4 whitespace-pre-wrap">
          {post.content}
        </div>
      </Card>
      ...
      <div className="pt-8 border-t border-[rgba(77,208,225,0.2)] space-y-6">
        <div>
          <p className="text-sm text-[rgba(232,230,227,0.5)] mb-4">
            By {post.author}
          </p>
          <Link
            href="/posts"
